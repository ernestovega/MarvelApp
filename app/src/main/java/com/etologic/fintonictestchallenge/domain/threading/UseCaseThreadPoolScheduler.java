...
package com.etologic.fintonictestchallenge.domain.threading;
...
class UseCaseThreadPoolScheduler implements UseCaseScheduler {
    ...
    @Override
    public <V extends UseCase.ResponseValue> void onError(final String strError,
                                                          final UseCase.UseCaseCallback<V> useCaseCallback) {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                useCaseCallback.onError(strError);
            }
        });
    }
